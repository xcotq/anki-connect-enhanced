...
### Building a Local Add-on Package ###

For a current Anki installation, build an installable local package with:

```bash
python build_addon.py
```

This generates `dist/AnkiConnectLocal.ankiaddon`. Install it in Anki via *Tools* → *Add-ons* → *Install from file*.
The package uses `ankiconnectlocal` as its local add-on folder name.

Configure the installed add-on via *Tools* → *Add-ons* → *AnkiConnect Local* → *Config*. Restart Anki after changing
the bind address, port, CORS origins, or API key.

The GitHub Actions workflow runs the Python 3.13 package tests, builds the `.ankiaddon` file, verifies its ZIP
integrity, and publishes it as a workflow artifact.
...
*   **batchEditNoteFields**
    ...
    Modify the fields of multiple existing notes in one request. Each item in `notes` uses the same `id` and `fields`
    structure as `updateNoteFields`.

    *Sample request*:
    ```json
    {
        "action": "batchEditNoteFields",
        "version": 5,
        "params": {
            "notes": [
                {
                    "id": 1514547547030,
                    "fields": {
                        "Front": "new first front content",
                        "Back": "new first back content"
                    }
                },
                {
                    "id": 1514547547031,
                    "fields": {
                        "Front": "new second front content"
                    }
                }
            ]
        }
