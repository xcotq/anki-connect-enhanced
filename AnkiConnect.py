...
class AjaxServer:
    def __init__(self, handler, address, port, backlog, corsOriginList):
        self.handler = handler
        self.address = address
        self.port = port
        self.backlog = backlog
        self.corsOriginList = corsOriginList
        self.clients = []
        self.sock = None
        self.resetHeaders()
    ...
    def handlerWrapper(self, req):
        self.extraHeaders = {}
        origin = req.headers.get(makeBytes('origin'))
        if origin is not None:
            origin = makeStr(origin)
            if '*' in self.corsOriginList or origin in self.corsOriginList:
...
class AnkiBridge:
    ...
    def addNote(self, params):
        ...
        self.injectAudio(note, params.audio)

        self.startEditing()
        deck = collection.decks.by_name(params.deckName)
        collection.add_note(note, deck['id'])
        self.stopEditing()

        return note.id

    def addNotes(self, paramsList):
        collection = self.collection()
        if collection is None:
            return

        results = [None] * len(paramsList)
        requests = []
        requestIndexes = []
        ...
        for index, request in zip(requestIndexes, requests):
            results[index] = request.note.id

        return results

    def injectAudio(self, note, audio):
        if audio is None or len(audio.fields) == 0:
            return
        ...
        if not skip:
            audioInject(note, audio.fields, audio.filename)
    ...
    def createNote(self, params):
        ...
        model = collection.models.by_name(params.modelName)
        if model is None:
            return
    ...
    def updateNoteFields(self, params):
        ...
        note = collection.get_note(params['id'])
        if note is None:
    ...
    def batchEditNoteFields(self, notes):
        collection = self.collection()
        if collection is None:
            return
        ...
        if updatedNotes:
            collection.update_notes(updatedNotes)
...
class AnkiConnect:
    def __init__(self):
        ...
        self.port = setting('webBindPort')
        self.anki = AnkiBridge()
        self.server = AjaxServer(
            self.handler,
            setting('webBindAddress'),
            self.port,
            setting('webBacklog'),
            setting('webCorsOriginList')
        )
        ...
        try:
            ...
            self.timer.start(setting('apiPollInterval'))
        except:
            QMessageBox.critical(
                self.anki.window(),
                'AnkiConnect',
                'Failed to listen on port {}.\nMake sure it is available and is not in use.'.format(self.port)
            )
    ...
    def handler(self, request):
        ...
        try:
            if self.apiKey is not None and request.get('key') != self.apiKey:
    ...
    def addNotes(self, notes):
        return self.anki.addNotes([AnkiNoteParams(note) for note in notes])
    ...
    @webApi()
    def batchEditNoteFields(self, notes):
        return self.anki.batchEditNoteFields(notes)
